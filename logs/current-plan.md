...
**Status: v1.0.0 (Phase 6), v1.1.0 (adapter backlog round 2), and v1.1.1 (nightly-refresh fix) all complete and live-verified. Live site: https://jameswardvp.github.io/RaceDates/. No active phase in progress — all work is now incremental adapter/data-quality sweeps.**

v1.1.1 follow-ups for a future session:
1. **Confirm the nightly Action actually goes green now** — it had failed silently for 5 days (2026-07-26 to 2026-07-30) before this fix; check the Actions tab after the next scheduled run (05:17 UTC) or trigger one manually via workflow_dispatch.
2. **Dismiss or resolve the "Lochgelly Raceway" duplicate discovery** on the admin page's review panel — it's very likely the same real venue as our existing Lochgelly Raceway track (which matched a differently-named Wikidata entity, "Driftland", at the same coordinates). James should look at it rather than me guessing.
3. If a *future* nightly run ever fails again, don't assume it's the same Wikidata rate-limiting cause — check the Actions tab's annotations first (bare "exit code 1" + a separate Node-deprecation warning is what a Wikidata block looks like; a different error means a different problem).
