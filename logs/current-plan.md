...
**Status: v1.0.0 (Phase 6), v1.1.0 (adapter backlog round 2), and v1.1.1+v1.1.2 (nightly-refresh fix, two rounds) all complete. v1.1.2's actual fix is verified locally against real `pwsh` 7.6.4 (installed this session specifically for this) but NOT yet confirmed on a live GitHub Actions run — that's the #1 follow-up below. Live site: https://jameswardvp.github.io/RaceDates/.**

v1.1.2 follow-ups for a future session:
1. **Confirm the nightly Action actually goes green now** — v1.1.1's root-cause diagnosis (Wikidata rate-limiting) was wrong; the real bug (a `[datetime]` vs `[string]` cross-version JSON-parsing difference between Windows PowerShell 5.1 and pwsh 7) is fixed in v1.1.2 and verified locally under real pwsh, but not yet on an actual GitHub Actions run. Check the Actions tab after the next scheduled run (05:17 UTC) or trigger one manually.
2. **`pwsh` is now installed locally** (`winget install Microsoft.PowerShell`, 7.6.4, resolves via `Get-Command pwsh` / lives at `%LOCALAPPDATA%\Microsoft\WindowsApps\pwsh.exe`) — going forward, verify `tools/*.ps1` changes against `pwsh -File ...` (the real CI runtime), not just the default Windows PowerShell 5.1 the `powershell` command opens. This whole two-round debugging saga happened because that gap existed; don't let it reopen.
3. **Dismiss or resolve the "Lochgelly Raceway" duplicate discovery** on the admin page's review panel (still pending from v1.1.1) — very likely the same real venue as our existing Lochgelly Raceway track (which matched a differently-named Wikidata entity, "Driftland", at the same coordinates).
4. If a *future* nightly run ever fails again: get the actual error text first (GitHub now requires sign-in for raw logs even on public repos, but a manual `workflow_dispatch` re-run's summary page shows it, or the Checks API annotations endpoint sometimes does) — don't diagnose from a bare "exit code 1" alone. That's what produced the wrong root-cause guess in v1.1.1.
