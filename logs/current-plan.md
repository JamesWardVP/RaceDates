...
**Status: v1.0.0 (Phase 6), v1.1.0 (adapter backlog round 2), and v1.1.1/v1.1.2/v1.1.3 (nightly-refresh fix, three rounds) all complete. The workflow now genuinely succeeds on real GitHub Actions (James confirmed), and a real data-loss bug the successful run itself exposed (v1.1.3) is fixed and the lost data recovered. Live site: https://jameswardvp.github.io/RaceDates/.**

v1.1.3 follow-ups for a future session:
1. **Castle Combe still hasn't produced a single successful scrape** — 0 events on both my local testing and the real GitHub Actions run. Could be their bot-protection blocking GitHub's runner IP ranges too, not just mine. Worth checking again after some time has passed rather than assuming it'll self-resolve; if it's still 0 after a few more nightly runs, revisit the adapter (maybe their protection needs more than just a browser UA — cookies, a referrer, etc.).
2. **Oliver's Mount's 6 events were restored from the v1.1.0 commit, not freshly re-verified** — they're real (not sample) data, just not re-confirmed as of 2026-07-30. Will self-correct whenever its adapter next succeeds; no action needed unless it's still failing after several more days.
3. Don't assume `Merge-VenueEvents`/`Merge-SeriesEvents` are the only two merge functions with this "empty result = data loss" risk if a new adapter pattern gets added later — check any new merge helper has the same "keep existing entries if the adapter returned nothing" guard before trusting it.

v1.1.1/v1.1.2 follow-ups still relevant:
1. **`pwsh` is now installed locally** (`winget install Microsoft.PowerShell`, 7.6.4, resolves via `Get-Command pwsh` / lives at `%LOCALAPPDATA%\Microsoft\WindowsApps\pwsh.exe`) — going forward, verify `tools/*.ps1` changes against `pwsh -File ...` (the real CI runtime), not just the default Windows PowerShell 5.1 the `powershell` command opens. The whole v1.1.1→v1.1.3 saga happened because that gap existed for months; don't let it reopen.
2. **Dismiss or resolve the "Lochgelly Raceway" duplicate discovery** on the admin page's review panel — very likely the same real venue as our existing Lochgelly Raceway track (which matched a differently-named Wikidata entity, "Driftland", at the same coordinates).
3. If a *future* nightly run ever fails again: get the actual error text first (GitHub now requires sign-in for raw logs even on public repos, but a manual `workflow_dispatch` re-run's summary page shows it). And even on a run that *succeeds*, it's worth spot-checking the committed data diff occasionally — v1.1.3's data-loss bug produced a "successful" green run while silently deleting real events.
