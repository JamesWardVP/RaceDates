...
4. **"Driftland" and "Lochgelly Raceway" are duplicate track entries** — identical Wikidata QID (Q114811655) and coordinates, filed as two separate tracks under two names. James needs to decide which name/id is canonical before merging (don't guess/auto-merge — could break existing links or bookmarks).
5. **25 tracks still have no adapter at all** (as opposed to a broken one) — see the expanded backlog list below. Real new-adapter research, bigger than a fallback-source addition.
6. Re-verify Isle of Man TT / Manx GP / Tandragee 100 dates around March–April 2027 and update the hand-entered events for next season (no adapter exists for these).
...
- **Multi-source fallback pattern (v1.1.5)**: `Get-EventsWithFallback "Label" @({ Get-PrimaryEvents }, { Get-SecondaryEvents })` tries each scriptblock in order, using the first that returns any events — wired in for BTCC/BSB so far, calling their site adapter first and `Get-WikipediaCalendarEvents` second. To add a fallback for another *series* with a Wikipedia season-calendar article that uses the same 3-column Round/Circuit/Date wikitable (check by fetching its wikitext and comparing structure — don't assume), it's just one more `Get-EventsWithFallback` call using the existing `Get-WikipediaCalendarEvents "<page title>" "<seriesId>" "<name prefix>"` — no new parsing code needed. A *venue* doesn't have a Wikipedia equivalent (Wikipedia covers championships, not local fixture calendars), so venue adapters would need a genuinely different second source researched per-venue, not this same pattern reused.
