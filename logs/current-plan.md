...
**Status: v1.0.0 through v1.1.7 all complete and verified. Live site: https://jameswardvp.github.io/RaceDates/. No active phase in progress.**

v1.1.7 (2026-07-30): both v1.1.5 follow-ups are done. British GT now has a Wikipedia fallback (own round-parser, its table layout differs from BTCC/BSB). Castle Combe has real committed data for the first time ever via a new `racingcalendar.net` fallback — 9 events, verified live while its own site was still blocking this machine. Also fixed a real latent bug found while building GT's fallback: unscoped Wikipedia parsing could pull a spurious extra "round" from an unrelated table further down the page — fixed for all three series (BTCC/BSB/GT), not just GT.
...
1. **`racingcalendar.net` could cover more "no adapter" venues, not just Castle Combe** — it's a general aggregator (Anglesey, Mallory Park, Kirkistown are all circuit-racing venues, the kind it seems to focus on) worth checking before writing a bespoke scraper for any of them. `Get-RacingCalendarNetEvents "<slug>" "<trackId>"` is already reusable, just needs the right slug per venue confirmed by checking `racingcalendar.net/circuit/<slug>/2026` loads real data first.
2. **"Driftland" and "Lochgelly Raceway" are duplicate track entries** — identical Wikidata QID (Q114811655) and coordinates, filed as two separate tracks under two names. James needs to decide which name/id is canonical before merging (don't guess/auto-merge — could break existing links or bookmarks).
3. **25 tracks still have no adapter at all** (as opposed to a broken one) — see the expanded backlog list below.
4. Re-verify Isle of Man TT / Manx GP / Tandragee 100 dates around March–April 2027 and update the hand-entered events for next season (no adapter exists for these).
...
- **Multi-source fallback pattern**: `Get-EventsWithFallback "Label" @({ Get-PrimaryEvents }, { Get-SecondaryEvents })` tries each scriptblock in order, using the first that returns any events. Wired in for BTCC/BSB/British GT (site adapter first, `Get-WikipediaCalendarEvents` second) and Castle Combe (site adapter first, `Get-RacingCalendarNetEvents` second).
- **Wikipedia fallback (series only)**: `Get-WikipediaCalendarEvents "<page title>" "<seriesId>" "<name prefix>" { Get-Wiki*Rounds @args }` — the last argument is whichever round-parser matches that series' actual table layout (`Get-WikiSeriesRounds` for BTCC/BSB's shape, `Get-WikiGtRounds` for GT's; a new series might need a third if its table differs from both — check the real wikitext before assuming). **Always scope to the calendar table first** via `Get-WikiCalendarTableText` (already built into the function) — confirmed live that skipping this lets the round-regex bleed into an unrelated table further down the same page and produce a spurious fake "round". Only covers national championship series; a venue has no Wikipedia equivalent.
- **RacingCalendar.net fallback (venues)**: `Get-RacingCalendarNetEvents "<slug>" "<trackId>"` — works with our normal `RaceDatesBot` UA (no browser-UA workaround needed, unlike Castle Combe's own site). Slug comes from the URL at `racingcalendar.net/circuit/<slug>/<year>` — confirm the page actually has real data before wiring in a new venue, it doesn't cover everything (no Santa Pod, for instance — the site leans toward circuit racing). Data is community-submitted per the site's own disclaimer, so treat it as fallback-only, not a source to prefer over an official site that's actually working.
...
- ~~BTCC~~ ✅ v1.1.4 — adapter was completely dead (site fully redesigned, old selectors don't exist anymore); rewritten against real current markup and verified against all 10 rounds including Croft/Silverstone. ✅ v1.1.5 added a Wikipedia fallback too — proved itself for real the same session, recovering Croft/Silverstone into committed data while btcc.net was still blocking this machine entirely.
...
- ~~Castle Combe~~ ✅ v1.1.7 — its own site still blocks every request from this machine (unresolved, own-site adapter left in place as primary in case it clears), but now has a working `racingcalendar.net` fallback: 9 real events, verified live in committed data and on its track page.
